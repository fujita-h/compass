...
export default function NavTab(props: Props) {
  const tabs2 = props.tabs2 ?? []
  const tabs = props.tabs.concat(tabs2)
  const router = useRouter()

  const handleSelectChanged = (e) => {
    const href = tabs.find((x) => x.name == e.target.value).href
    if (href) {
      router.push(href)
    }
  }
    ...
    <div>
      ...
      <div className="hidden sm:block">
        <div className="flex justify-between border-b border-gray-200">
          ...
          <nav className="-mb-px flex space-x-8 last:pr-8" aria-label="Tabs">
            {tabs2.map((tab) => (
              <span key={tab.name}>
                <Link href={tab.href} passHref>
                  <a
