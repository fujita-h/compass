        ...
        <thead className="bg-gray-50">
          <tr>
            ...
            <th scope="col" className="text-end px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
              {group?.user_group_map.length} 人のメンバー
            </th>
          </tr>
        </thead>
...
const DeleteUserModal = ({ auth, state, setState }) => {
  ...
  return (
    <MyModal show={state.show} close={closeModal} title="メンバーの削除">
      <div className="mt-2">
        <p className="text-sm text-gray-500">以下を確認してください。</p>
        <ul className="text-sm text-gray-500">
          <li>
            ユーザー {state.user?.User?.username} を グループ {state.group?.name} から削除しようといています
          </li>
          <li>
            ユーザー {state.user?.User?.username} が グループ {state.group?.name} に作成した記事は削除されず残ります
          </li>
        </ul>
      </div>
