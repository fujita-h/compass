...
export default function Page() {
  ...
  return (
    <Layout showFooter={false}>
      <InnerPage userId={session.id} documentId={documentId} />
    </Layout>
  )
}
...
const InnerPage = ({ userId, documentId }: { userId: string; documentId: string }) => {
  ...
  const submitButtonMap: Array<SubmitButtonSetting> = [
    { key: 'publish', label: 'ドキュメントを更新' },
    { key: 'draft', label: '下書きに保存' },
  ]

  if (loadingDocument) {
    return (
      <EditorForm data={{ title: '', body: '', tags: [] }} meta={{}} submitButtonMap={submitButtonMap} submitType="dummy" loading={true} />
    )
  }
  if (userId !== document.document.paper.user.id) {
  ...
  if (document.drafts && document.drafts.length > 0) {
    return (
      <div className="mx-auto mt-5 max-w-7xl">
        <div className="border-b-1 border-gray-300 text-xl">
          同じドキュメントの下書きがあるため、このドキュメントを編集できません。以下の下書きから編集を再開して下さい。
        </div>
        <div className="mt-4 ml-4 text-lg">
          {document.drafts.map((draft) => (
            <div key={`draft-ref-${draft.id}`}>
  ...
  return (
    <EditorForm
      data={{
        ...
        tags: document.document.paper.tags.split(',').filter((tag) => tag !== ''),
      }}
