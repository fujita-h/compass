...
export const NavDrafts = ({ current }: { current?: string }) => {
  const { data, loading } = useDraftsQuery({ variables: { auth: 'user' } })
  const [filter, setFilter] = useState('')
  if (loading) return <></>
  if (!data) return <></>

  const groups = data.drafts.map((draft) => draft.group).filter((elem, idx, self) => self.findIndex((e) => e.id === elem.id) === idx)
    ...
    <>
      ...
      <nav className="space-y-1" aria-label="Sidebar">
        <ul role="list" className="relative z-0 divide-y divide-gray-200 border-y border-gray-200">
          {groups
            .filter((group) => group.name.includes(filter))
            .map((group) => {
              const isCurrent = current ? current.toLowerCase() === group.id.toLowerCase() : false
              return (
                <li key={group.id}>
                  <div
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <NextLink href={`/drafts?groupId=${encodeURIComponent(group.id.toLowerCase())}`} className="focus:outline-none">
                        {/* Extend touch target to entire panel */}
                        <span className="absolute inset-0" aria-hidden="true" />
                        <p className="text-sm font-medium text-gray-900">{group.name}</p>
                        <p className="truncate text-sm text-gray-500">
                          {data.drafts.filter((x) => x.group.id == group.id).length} 件のドキュメント
                        </p>
                      </NextLink>
                    </div>
                  </div>
