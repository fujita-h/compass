...
export function useSession({ redirectTo, redirectIfFound }: { redirectTo?: string; redirectIfFound?: boolean } = {}) {
...
export function useAdminSession({ redirectTo, redirectIfFound }: { redirectTo?: string; redirectIfFound?: boolean } = {}) {
  const { data, error } = useSessionQuery()
